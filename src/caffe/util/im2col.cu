...
template <typename Dtype>
__global__ void im2col_gpu_kernel_relu(const int n, const Dtype* data_im,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
...
template <typename Dtype>
__global__ void im2col_gpu_kernel_norm(const int n, const Dtype* data_im,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    data_col_ptr += (c_col * height_col + h_col) * width_col + w_col;
    const Dtype* data_im_ptr = data_im;
    Dtype* relu_im_ptr = relu_;
    data_im_ptr += (c_im * height + h_offset) * width + w_offset;
    relu_im_ptr += (c_im * height + h_offset) * width + w_offset;

    Dtype gamma = gamma_[c_im];
    Dtype beta = beta_[c_im];

    int h_im = h_offset;
    int w_im = w_offset;
    if ((h_im >= 0 && w_im >= 0 && h_im < height && w_im < width)) {
      *relu_im_ptr = (*data_im_ptr) * gamma + beta;
    }

    if (h_col > (height_col - 2) || w_col > (width_col - 2))
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        int h_im = h_offset + i * dilation_h;
        int w_im = w_offset + j * dilation_w;
        if (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) {
          relu_im_ptr[i * dilation_h * width + j * dilation_w] = data_im_ptr[i * dilation_h * width + j * dilation_w] * gamma + beta;
        }
      }
    }
    ...
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        ...
        *data_col_ptr =
            (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ?
            (data_im_ptr[i * dilation_h * width + j * dilation_w]) * gamma + beta : 0;
        if (*data_col_ptr < 0) {
          *data_col_ptr = 0;
        }
        data_col_ptr += height_col * width_col;
...
void im2col_gpu_relu(const Dtype* data_im, const int channels,
  ...
  int num_kernels = channels * height_col * width_col;
  // NOLINT_NEXT_LINE(whitespace/operators)
  im2col_gpu_kernel_relu<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                             CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, data_im, height, width, kernel_h, kernel_w, pad_h,
      pad_w, stride_h, stride_w, dilation_h, dilation_w, height_col,
      width_col, data_col);
...
// norm fusion for non-1x1 kernel
template <typename Dtype>
void im2col_gpu_norm(const Dtype* data_im, const int channels,
  ...
  int num_kernels = channels * height_col * width_col;
  // NOLINT_NEXT_LINE(whitespace/operators)
  im2col_gpu_kernel_norm<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                             CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, data_im, height, width, kernel_h, kernel_w, pad_h,
      pad_w, stride_h, stride_w, dilation_h, dilation_w, height_col,
      width_col, data_col,
      mean_, var_, gamma_, beta_, relu_);
  CUDA_POST_KERNEL_CHECK;
}
...
// Explicit instantiation
template void im2col_gpu_relu<float>(const float* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w, float* data_col);
template void im2col_gpu_relu<double>(const double* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w, double* data_col);

// Explicit instantiation for fusion
template void im2col_gpu_norm<float>(const float* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w, float* data_col,  const float* mean_, const float* var_, const float* gamma_, const float* beta_, float* relu_ptr);
template void im2col_gpu_norm<double>(const double* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w, double* data_col,  const double* mean_, const double* var_, const double* gamma_, const double* beta_, double* relu_ptr);
...
template <typename Dtype>
__global__ void col2im_gpu_kernel_gather(const int n, const Dtype* data_col,
    const int height, const int width, const int channels,
    const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    ...
    const int height_col, const int width_col,
    Dtype* data_im,
    const Dtype* mean, const Dtype* var_, const Dtype* x_relu, const Dtype* x_bn, Dtype* d_gamma, Dtype* d_beta) {
  CUDA_KERNEL_LOOP(index, n) {
    Dtype val = 0;
    const int w_im = index % width + pad_w;
    const int h_im = (index / width) % height + pad_h;
    ...
    Dtype temp = x_relu[index] > 0? val : 0;
    data_im[index] = temp;

    // C * H * W
    d_beta[index] += temp;
    d_gamma[index] += temp * (x_bn[index] - mean[c_im]) / var_[c_im];
  }
}
...
void col2im_gpu_gather(const Dtype* data_col, const int channels,
    ...
    const int stride_w, const int dilation_h, const int dilation_w,
    Dtype* data_im,
    const Dtype* mean, const Dtype* var, const Dtype* x_relu, const Dtype* x_bn, Dtype* d_gamma, Dtype* d_beta) {
  int height_col = (height + 2 * pad_h - (dilation_h * (kernel_h - 1) + 1)) /
      stride_h + 1;
  int width_col = (width + 2 * pad_w - (dilation_w * (kernel_w - 1) + 1)) /
      stride_w + 1;
  ...
  // bottom dimension, and then in the kernel add up the top dimensions.
  // NOLINT_NEXT_LINE(whitespace/operators)
  col2im_gpu_kernel_gather<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                             CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, data_col, height, width, channels, kernel_h, kernel_w,
      pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w,
      height_col, width_col, data_im, mean, var, x_relu, x_bn, d_gamma, d_beta);
  CUDA_POST_KERNEL_CHECK;
}
...
template void col2im_gpu_gather<float>(const float* data_col, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w,
    float* data_im,
    const float* mean, const float* var, const float* x_relu, const float* x_bn, float* d_gamma, float* d_beta);
template void col2im_gpu_gather<double>(const double* data_col, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w,
    double* data_im,
    const double* mean, const double* var, const double* x_relu, const double* x_bn, double* d_gamma, double* d_beta);
